...
export default function LoginPage() {
  ...
  const validateForm = useCallback(() => {
    ...
    if (!formData.userId.trim()) {
      errors.userId = 'Por favor, insira seu ID de usuário.';
    } else if (formData.userId.length > 8) {
      errors.userId = 'O ID do usuário deve ter até 8 caracteres.';
    }
  ...
  const getErrorMessage = (error: string) => {
    const errorMappings: Record<string, string> = {
      ...
      'Network error occurred': 'Não foi possível verificar as credenciais. Cheque sua conexão.',
    };
      ...
      <Box onKeyDown={handleKeyDown} tabIndex={-1}>
        ...
        <Box sx={{ position: 'relative' }}>
          <SystemHeader
            ...
            subtitle="Modernização Mainframe"
            showNavigation={false}
          />
          
          {/* ✅ NUEVO: Botón de documentação discreto en el header */}
          <Box
            sx={{
              position: 'absolute',
              top: 8,
        ...
        >
          ...
          >
            ...
            <Typography variant="h4" fontWeight={600} gutterBottom>
              NOTA RESERVA NACIONAL
            </Typography>
            <Typography variant="h6" sx={{ opacity: 0.9 }}>
              OS ESTADOS UNIDOS DE KICSLAND
            </Typography>
            ...
            >
              {`+========================================+
|%%%%%%%  NOTA RESERVA NACIONAL  %%%%%%%%|
|%(1)  OS ESTADOS UNIDOS DE KICSLAND (1)%|
|%$$              ___       ********  $$%|
|%$    {x}       (o o)                 $%|
|%$     ******  (  V  )      U M     $%|
|%(1)          ---m-m---             (1)%|
          ...
          <Box sx={{ p: 4 }}>
            ...
            >
              <Stack spacing={3}>
                ...
                >
                  {isLoading ? 'Efetuando login...' : 'ENTER = Entrar'}
                </Button>
              </Stack>
            </Box>
